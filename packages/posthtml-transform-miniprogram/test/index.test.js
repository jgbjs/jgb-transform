...
describe("wx => aliapp", () => {
  ...
  test("wx:if => a:if", async () => {
    await expect(`<view wx:if="{{data}}"></view>`).toBeAliapp(
      `<view a:if="{{data}}"></view>`
    );
  });
  ...
  test("<scroll-view> event", async () => {
    await expect(
      `<scroll-view bindscrolltoupper="bindscrolltoupper" bindscrolltolower="bindscrolltolower" bindscroll="bindscroll"></scroll-view>`
    ).toBeAliapp(
      `<scroll-view onScrollToUpper="bindscrolltoupper" onScrollToLower="bindscrolltolower" onScroll="bindscroll"></scroll-view>`
    );
  });
  ...
  test("ali origin event like will not tranform", async () => {
    ...
    </view>`).toBeAliapp(`<view onTap="bind">
      <view class="chooseCar" onTap="bindTap" catchTap="catchTap"></view>
      </view>`);
  });
  ...
  test("wx event bind style", async () => {
    await expect(
      `<view bind:tap="bindTap" catch:tap="catchTap"></view>`
    ).toBeAliapp(`<view onTap="bindTap" catchTap="catchTap"></view>`);
    await expect(
      `<view bindtap="bindTap" catchtap="catchTap"></view>`
    ).toBeAliapp(`<view onTap="bindTap" catchTap="catchTap"></view>`);
  });
  ...
  test(`<wxs> to <import-sjs> inline script`, async () => {
    ...
    </wxs>`).toBeAliapp(`<import-sjs name="tools">
      var foo = "'hello world' from tools.wxs";
      var bar = function (d) {
        return d;
      }
  ...
  test(`<wxs> to <import-sjs> src`, async () => {
    await expect(`<wxs src="./../tools.wxs" module="tools" />`).toBeAliapp(
      `<import-sjs name="tools" from="./../tools.wxs"></import-sjs>`
    );
  });
});
...
describe("wx => swan", () => {
  test("wx:if => s-if", async () => {
    await expect(`<view wx:if="{{data}}"></view>`).toBeSwan(
      `<view s-if="{{data}}"></view>`
    );
  });
  ...
  test("wx:for => s-for", async () => {
    await expect(
      `<view wx:for="{{data}}" wx:for-item="item" wx:for-index="idx" wx:key="key"></view>`
    ).toBeSwan(
      `<view s-for="{{data}}" s-for-item="item" s-for-index="idx" s-key="key"></view>`
    );
  });
  ...
  test(`template: {{}} => {{{}}}`, async () => {
    await expect(`<template is="msgItem" data="{{...item}}" />`).toBeSwan(
  ...
  test(`wxs to import-sjs`, async () => {
    await expect(`<wxs src="./../tools.wxs" module="tools" />`).toBeSwan(
      `<import-sjs src="./../tools.wxs" module="tools"></import-sjs>`
    );
  });

  test(`remove image webp`, async () => {
    await expect(`<image src="xxx" webp="true"></image>`).toBeSwan(
      `<image src="xxx"></image>`
    );
  });
});
...
describe("wx => tt", () => {
  test("wx:if => tt:if", async () => {
    await expect(`<view wx:if="{{data}}"></view>`).toBeToutiao(
      `<view tt:if="{{data}}"></view>`
    );
  });

  test("wx:for => tt:for", async () => {
    await expect(`<view wx:for="{{array}}"></view>`).toBeToutiao(
      `<view tt:for="{{array}}"></view>`
    );
  });
});
